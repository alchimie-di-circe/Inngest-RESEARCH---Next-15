---
tags: [agentkit, streaming, guides, all-agents]
description: Building UI with useAgent streaming
alwaysApply: false
---
...
use-agent Example
Find the complete source code for a Next.js chat application using useAgent on GitHub.
​
How it Works
...
Client Initialization: The useAgent hook is initialized in your React component. It uses the useInngestSubscription hook from @inngest/realtime/hooks to establish a WebSocket connection.
Authentication: To connect, it calls a backend API route (e.g., /api/realtime/token) to get a short-lived subscription token. This token authorizes the client to listen to events on a specific user channel.
Sending a Message: The user types a message and the UI calls the sendMessage function returned by the hook.
API Request: sendMessage makes a POST request to a backend API route (e.g., /api/chat). This request contains the message content, the current threadId, and the conversation history.
Triggering Inngest: The chat API route receives the request and sends an agent/chat.requested event to Inngest using inngest.send().
Agent Execution: An Inngest function (run-agent-chat.ts in our example) is triggered by the event. It sets up the AgentKit network, state, and history adapter.
Running the Network: The Inngest function calls network.run(message, { streaming: ... }).
Streaming Events: As the network and its agents execute, they generate streaming events (e.g., run.started, part.created, text.delta). The streaming.publish function inside the Inngest function forwards these events back to the user’s realtime channel.
Realtime Push: The events are pushed over the WebSocket connection to the client.
State Update: useAgent receives the raw events, processes them in the correct sequence, handles out-of-order events, and updates its internal state.
UI Re-render: The component using the hook re-renders with the new messages, agent status, and other UI parts.
​
Sequence Diagram
...
loop
[Event Streaming]
Initialize with threadId, userId
POST /api/realtime/token
Subscription Token
Establishes WebSocket Connection
sendMessage("Hello!")
POST /api/chat (message, history)
inngest.send("agent/chat.requested")
Triggers Inngest Function (run-agent-chat.ts)
network.run(message, { streaming })
publish(event)
Pushes event via WebSocket
Processes event, updates state
