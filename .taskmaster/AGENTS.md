...
- `.taskmaster/AGENTS.md` - Auto-loaded context for all cli dev agents (this file)
- `.*/commands/tm/` - Custom slash commands for repeated workflows
- `mcp.json` or 'settings.json' (depending on the cli dev agent) - MCP server configuration (project-specific)
...
Task Master provides an MCP server that AI agents can connect to. Configure in `mcp.json` or other config files provider specific:
...
Default: `all` (44+ tools). Set via `TASK_MASTER_TOOLS` env var.
...
**Change settin if needed:** Edit MCP config, change `TASK_MASTER_TOOLS` from `"core"` to `"standard"` or `"all"`, restart MCP.
...
## Dev agents Workflow Integration
...
For complex projects, use multiple agent sessions:
...
cd project-test-worktree && codex

# Terminal 3: Documentation updates
cd project-docs-worktree && kilocode
```
...
At least **one** of these API keys must be configured (only if user did not log in with Claude/Codex/Gemini account):
...
## Best Practices with Task Master
...
# Run an agent in each worktree
cd ../project-auth && gemini    # Terminal 1: Auth work
cd ../project-api && droid     # Terminal 2: API work
```
...
task-master models --set-fallback gemini cli... 
```
