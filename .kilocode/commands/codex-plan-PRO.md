...
1. Understand the user's planning request between 2 options: prd-plan (full stack app, Greenfield or started/blocked/broken), taskmaster task plan (.taskmaster/tasks/*.* ; single .md task file + complete tasks.json file 
2. Ask clarifying questions using to be sure which option the user wants (check for tagged files: if it's a single task file you have to do a taskmaster task plan, if it's the tasks.json file (with all the tasks) or a prd.md/prd.txt file make a pre-plan ) and to improve plan quality
3. Craft an excellent, detailed prompt for Codex
4. Execute Codex to generate and save the plan (check before the branch and STOP + ask the user if the current session is on the main branch
...
## Step 1: Analyze the Request: 
	...
	- if it's a taskmaster task plan request verify/refine/expand all the subtask of the tagged task making 1 prompt for codex (to search the web on official docs and Reddits related to the subtask) for each subtask, and then spawn one codex headless command (with the subtask research prompt file you just created at .taskmaster/docs/subtask-*-codex-search.md) for each subtask. Monitor all the codex headless searches, after each is finished, collect every finding, analyze and verify everything is correct and joint everything in a perfect taskmaster-task-*-codex-plan.md 

	- if it's a complete prd-plan then follow strictly this workflow:

Look at the tagged/mentioned prd.md file. Identify:
